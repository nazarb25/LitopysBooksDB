...
from litopysdb.application.use_cases import SearchBooks
from litopysdb.presentation.schemas import BookRead, SearchResponse
...
def create_app(search_books: SearchBooks) -> FastAPI:
    app = FastAPI(title="Літопис книг — пошук")
    ...
    def books(
        ...
        total = search_books.count(query)
        return SearchResponse(
            items=[BookRead.from_book(item) for item in search_books.execute(query)],
            total=total,
            limit=limit,
            offset=offset,
            page=offset // limit + 1,
        ...
        if any(
            ...
            try:
                ...
                if current_page != page:
                    query.offset = (current_page - 1) * query.limit
                found = search_books.execute(query)
                cards = []
                for book in found:
