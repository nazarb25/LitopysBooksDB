...
def test_catalog_filters_book_pdfs_and_handles_old_names(monkeypatch) -> None:
    html = b"""<a href="litopys/Knigki/2004/L_K_1_04.pdf">1</a>
    <a href="/litopys/Knigki/2013/Lk_4_2013%20.pdf">4</a>
    ...
    client = BookChamberClient(delay=0)
    monkeypatch.setattr(client, "_get", lambda _: html)
    try:
        issues = client.list_issues()
    finally:
        client.close()

    assert [(issue.year, issue.number) for issue in issues] == [
        (2004, 1),
        (2013, 4),
        (2026, 16),
    ]
